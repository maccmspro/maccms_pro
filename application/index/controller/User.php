...
class User extends Base
{
    ...
    public function ajax_ulog()
    {
        ...
        if ($param['ac'] == 'set') {
            ...
            if ($data['ulog_points'] == 0) {
                ...
                $res['ulog_id'] = model('Ulog')->getLastInsID();
            } else {
        ...
        $res['msg'] = lang_web('saved_successfully');
        return json($res);
    }
    ...
    public function portrait()
    {
        if(request()->isPost()){
            ...
            if ($res['code'] == 1) {
                cookie('user_portrait', mac_get_user_portrait(), ['expire' => 2592000]);
            }
            return json($res);
        }
    ...
    public function gopay()
    {
        ...
        if (empty($order_code) && empty($order_id) && empty($payment)) {
            return $this->error(lang_frontend('param_err'));
        }

        if ($GLOBALS['config']['pay'][$payment]['appid'] == '') {
            return $this->error(lang_frontend('index/payment_status'));
        }
        ...
        $res = model('Order')->infoData($where);
        if ($res['code'] > 1) {
            return $this->error(lang_frontend('index/order_not'));
        }
        if ($res['info']['order_status'] == 1) {
            return $this->error(lang_frontend('index/order_payed'));
        }
