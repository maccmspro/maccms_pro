...
function get_supported_lang_list() {
    $langs = glob(APP_PATH . 'lang/*.php');
    foreach ($langs as $k => &$v) {
        $v = str_replace([APP_PATH . 'lang/', '.php'], '', $v);
    }
    return $langs;
}

function get_url_pre($str) {
    $final = $str;
    if (substr($str, -4) != '.php') {
        $final = $str . '/index.php';
    }
    // 兼容基础请求
    if (!in_array(request()->root(), ['', '/', '/index.php'])) {
        $final = $final . '?s=';
    }
    return $final;
}
...
function config_db_set($name, $data) {
    $GLOBALS['config_db_' . $name] = $data;
    $data = serialize($data);
    $model = model('Config', 'common\model');
    $config_id = $model->getValueByCond("config_key='{$name}'", "config_id");
    if (empty($config_id)) {
        $model->rawInsert(['config_key' => $name, 'config_value' => $data]);
    } else {
        $model->rawUpdate($config_id, ['config_value' => $data]);
    }
}
...
if (!function_exists('config')) {
    ...
    function config($name = '', $value = null, $range = '')
    {
        if (is_null($value) && is_string($name)) {
            $final = 0 === strpos($name, '?') ? Config::has(substr($name, 1), $range) : Config::get($name, $range);
            // 支持配置由数据库读取
            if (Config::get('config_save_in_db') && !empty($config_db = config_db_get($name))) {
                // 纯列表型的不合并，以数据库为准
                if (array_keys_are_sorted_number($config_db)) {
                    $final = $config_db;
                } else {
                    $final = array_merge($final, $config_db);
                }
            }
            return $final;
        } else {
 ...
 * 是否纯数字数组
 */
function array_keys_are_sorted_number($array) {
    $keys = array_keys($array);
    $length = count($keys);
    if ($length > 0 && $keys[0] == 0 && $keys[$length - 1] == $length - 1) {
        return true;
    }
    return false;
}
...
{
    if (Config::get('config_save_in_db')) {
        $file_exploded = explode('.', basename($f));
        array_pop($file_exploded);
        $config_key = join('_', $file_exploded);
        config_db_set($config_key, $arr);
    } else {
        mac_arr2file($f, $arr);
    }
}
...
function mac_get_plog_type_text_web($data)
{
    $arr = [1=>lang_web('integral_recharge'),2=>lang_web('registration_promotion'),3=>lang_web('visit_promotion'),4=>lang_web('one_level_distribution'),5=>lang_web('two_level_distribution'),6=>lang_web('three_level_distribution'),7=>lang_web('points_upgrade'),8=>lang_web('integral_consumption'),9=>lang_web('integral_withdrawal')];
    return $arr[$data];
}

function mac_get_plog_type_text_web_data_lang($data)
{
    $arr = [1=>'integral_recharge',2=>'registration_promotion',3=>'visit_promotion',4=>'one_level_distribution',5=>'two_level_distribution',6=>'three_level_distribution',7=>'points_upgrade',8=>'integral_consumption',9=>'integral_withdrawal'];
    return $arr[$data];
}
 ...
 * @return string
 */
function to_byte_string($size, $precision = 2) {
