...
class User extends Base
{
    ...
    public function login($param)
    {
        ...
        if (!empty($row['user_end_time']) && $row['user_end_time'] > time()){
            cookie('is_member', true, ['expire'=>2592000]);
        }else{
            cookie('is_member', false, ['expire'=>2592000]);
        }
    ...
    public function upgrade($param)
    {
        ...
        if (!array_key_exists($long, $points_long)) {
            return ['code'=>1001,'msg'=>lang('model/user/access_denied')];
        }
