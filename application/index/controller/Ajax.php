...
class Ajax extends Base
{
    ...
    public function suggest()
    {
        ...
        $array = explode(",",$config['app']['search_hot']);

        $res['site_keywords'] = $array;
    ...
    public function score()
    {
        ...
        if(empty($id) ||  !in_array($mid,['1','2','3','8','9','11']) ) {
            return json(['code'=>1001,'msg'=>lang_frontend('param_err')]);
        }
        ...
        if ($info) {
            if($score){
                $cookie = $pre.'-score-'.$id;
                if(!empty(cookie($cookie))){
                    return json(['code'=>1002,'msg'=>lang_frontend('index/haved')]);
                }
                $update=[];
    ...
    public function watch_progress()
    {
        $user_id = $GLOBALS['user']['user_id'];
        if (!$user_id) {
            return json(['code' => 0,'msg' => lang_frontend('index/ajax/watch_progress/user_login')]);
        }

        $param = input();

        $ulog_rid = (int) $param['vod_id'];
        $ulog_watch_progress = (int) $param['watch_progress'];
        $ulog_sid = (int) ($param['sid']?$param['sid']:0);
        $ulog_nid = (int) ($param['nid']?$param['nid']:0);

        if (!isset($ulog_rid) || !isset($ulog_watch_progress)) {
            return json(['code' => 0,'msg' => lang_frontend('index/ajax/watch_progress/param_error')]);
        }

        if ($ulog_watch_progress < 0 || $ulog_watch_progress > 100) {
            return json(['code' => 0,'msg' => lang_frontend('index/ajax/watch_progress/param_lang_small')]);
        }
        ...
        if ($result) {
            ...
            return json(['code' => 0,'msg' => lang_frontend('index/ajax/watch_progress/db_error')]);
        }
    }
}