...
function gold_init(){
	$("ul.rating-star li").each(function(i) {
		...
		$(this).click(function () {
				if (hadpingfen > 0) {
					$.showfloatdiv({txt: lang==1?'Already scored, please repeat the score':'已经评分,请务重复评分'});
					// $.hidediv({});
				}else {
					$.showfloatdiv({txt: lang==1?'Data submission in progress...': '数据提交中...', cssname: 'loading'});
					$lis.removeClass("active");
					$("ul.rating li:lt(" + n + ")").addClass("active");
					$("#ratewords").html($title);
					$.getJSON(maccms.base_url+'/index.php/ajax/score?mid='+$('#rating').attr('data-mid')+'&id='+$('#rating').attr('data-id')+'&score='+($(this).attr('val')*2), function (r) {
