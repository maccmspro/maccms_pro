...
class System extends Base
{
    ...
    public function config()
    {
        if (Request()->isPost()) {
            ...
            //同步更新公告设置   walle
            if (!empty($config["site"]["site_announcement"])){
                $mctheme_config = config("mctheme");
                $mctheme_config["theme"]["tips"]["text"] = $config["site"]["site_announcement"];
                mac_save_config_data(APP_PATH . 'extra/mctheme.php', $mctheme_config);
            }
            if ($res === false) {
...
//                    }
            ...
            $res = mac_save_config_data(APP_PATH . 'route.php', $route);
            if ($res === false) {
